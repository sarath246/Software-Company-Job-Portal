<?php
		  ...
		  if(isset($_POST['btnsave']))
	{
		 ...
		 $freelancer=$_SESSION['freelancerid'];
		   //Proof
		   
          $proof=$_FILES["fleResume"]["name"];
		  
		  $temp=$_FILES["fleResume"]["tmp_name"];
		  move_uploaded_file($temp,"../Freelancer/Proof/".$proof);
		   
		 
         $ins="insert into tbl_jobapply(freelancer_id,jobapply_date,jobvacancy_id,upload_resume,company_id)
		 values('".$_SESSION['freelancerid']."','".$ApplyDate."','".$_GET['jobvacid']."','".$proof."','".$comp."')";
		    ...
		    if(isset($_POST['submit']))
     {
		 ...
		 $category=$_POST['jobcategory_id'];
		  ...
		  $insert="insert into tbl_save(company_id,jobvacancy_id,freelancer_id,jobapply_date,jobcategory_id,qualification_id,user_exp,vacancy_no)
		 values('".$comp."','".$jobvacancy."','".$freelancer."','".$ApplyDate."','". $category."','".$qualif."','".$experience."','".$vacancynum."')";
		// echo $insert;
...
?>
    ...
    <tr>
      ...
      <td width="226">
      <input type="text" name="txt_company" hidden="company_id" value="<?php echo $data['company_id']; ?>" />
      <div class="w3-container">
